...
- **Prioriteit:** middel
- **Status:** deel (A) ✅ uitgevoerd (v0.26). **Deel (B) is vervallen — niet
  meer oppakken.** Het is gebouwd, getest en daarna op verzoek van de
  eigenaar weer volledig verwijderd (revert van commit `5358a6e`). Zie
  "Deel (B): waarom dit vervallen is" hieronder — die redenering is
  bewaard omdat ze óók iets zegt over het bestaande barricadesysteem.
  ...
  *1. De premisse van het ticket klopte niet* (gemeten vóór de bouw,
  ...
  - `golfSpawnStap()` beukt uitsluitend op `VENSTERS`, en die lijst bevat
    **twee** ramen — beide in dezelfde muur, 4 m uit elkaar, in de
    ...
    worden nooit gebeukt. Er zijn dus 6 beukbare planken in het hele pand,
    geen "rondje door het gebouw".
  - Die 6 planken **sneuvelen allemaal in golf 1**. Golf 2 t/m 25: 0
    planken, 0 beuk-events. De barricademechaniek is in de praktijk een
    golf-1-mechaniek, en een herstelknop had ná golf 1 niets te herstellen.
  - Handmatig repareren **levert €20 per plank op**, dus €120 voor alles.
    Een betaalde "herstel alles" zou strikt gedomineerd zijn door de
    gratis variant die je juist geld oplevert.

  *2. De vervangende vorm was mechanisch gezond maar niet interessant.* Er
  is daarna een herbruikbaar versterkingspunt gebouwd (barricades naar 6
  planken voor een oplopende prijs, 400 × 1,6 per gebruik) dat alle
  ticket-eisen haalde: gemeten tempo-winst (0 in plaats van 4 ondoden na
  10 s golf op golf 20), een prijs die zichzelf uit de markt prijst, en
  een blokkade op de `ONTSNAPPING_PRIJS`-drempel. 32 checks groen. Bij de
  oplevering bleek echter een gat dat geen test vangt: **een speler die
  barricades negen golven lang terecht genegeerd heeft, heeft geen enkele
  reden om te geloven dat "6 planken" €400 waard is** — planken zijn na
  golf 1 onzichtbaar, dus de waarde van de aankoop is niet te leren. De
  eigenaar heeft de mechaniek daarop als "niet boeiend" afgevoerd. Dat is
  een productoordeel, geen bug; de code is teruggedraaid in plaats van
  doorontwikkeld.

  **Wat dit betekent voor het onderliggende probleem.** Het gat dat deel
  (B) moest dichten staat nog open: boven ongeveer €3.333 restsaldo levert
  extra geld niets meer op (daar plafonneert deel (A)'s score-conversie),
  en er is geen herbruikbare uitgave die met rijkdom meeschaalt. Wie dit
  alsnog wil aanpakken: begin met een NIEUW ticket en een ander voertuig
  dan barricades — en let op de les hierboven, namelijk dat een geldput
  alleen werkt als de speler de waarde ervan kán leren zien.

  **Losse bevinding, bewaard.** Het barricadesysteem zelf is feitelijk
  dode content na golf 1 (zie de meting hierboven). Dat is los van dit
  ticket een keuze waard: laten zoals het is, leesbaarder maken, of
  weghalen. Nu is het geen van drieën bewust.
...
- **Sonnet solo:** deel (A) ja. Deel (B) nee — en dat is precies zo
  uitgepakt: deel (B) haalde elke geautomatiseerde eis (32 checks groen)
  en sneuvelde alsnog op het speelsessie-oordeel dat hier voorbehouden
  werd. De oorspronkelijke redenering: een nieuwe herbruikbare geldput
  raakt de rondebalans en verdient een speelsessie-oordeel, geen puur
  headless groen vinkje.
...
### Ticket 175 (backlog) — De chirurgische assetuitzondering
- **Status:** backlog — **idee, niet uitvoeren zonder expliciete opdracht.**
  Op verzoek van de eigenaar van een actieve ronde naar de backlog verplaatst,
  nog vóór er iets van gebouwd was.
- **Wat er WEL al gebeurd is, en dus blijft staan:** het regelbesluit. De
  eigenaar heeft `CLAUDE.md` laten aanpassen, dus de uitzondering is formeel
  van kracht: ingesloten audiofragmenten (base64, nooit een los bestand) voor
  uitsluitend menselijke stem, mits geregistreerd in `ASSETS.md` met bron,
  auteur, licentie, datum en SHA-256. Alle overige audio blijft procedureel,
  de uitzondering geldt niet voor beeld, en de IP-regels blijven onverkort.
- **Wat er NIET gebouwd is:** alles. Geen `ASSETS.md`, geen
  `test-assetregister.mjs`, geen `test-ondode-stemmen.mjs`, geen laadpad, geen
  opnamen.
- **LET OP — de permissie loopt vooruit op de handhaving.** De regel staat in
  `CLAUDE.md`, maar de test die hem bewaakt bestaat nog niet. Zolang dat zo is,
  is de enige bescherming dat niemand er gebruik van maakt. Wie dit ticket ooit
  oppakt: **bouw eerst `ASSETS.md` plus `test-assetregister.mjs`, en pas daarna
  de eerste opname.** Andersom bouw je de grens pas nadat hij al een keer
  overschreden is. Wie de regel liever weer dichtzet tot het zover is, haalt de
  twee regels uit `CLAUDE.md` onder "Werkwijze" — daar hangt verder niets aan.
- **De volledige onderbouwing** (afbakening, het ontwerpprincipe dat de
  procedurele grom de permanente terugval blijft, het variatiebudget, de
  risico's, acceptatiecriteria en testplan) staat hieronder ongewijzigd
  bewaard, zodat oppakken geen heranalyse vraagt.

<details>
<summary>Volledige ticketuitwerking (v0.33-voorstel, ongewijzigd bewaard)</summary>

#### Ticket 175 — De chirurgische assetuitzondering (oorspronkelijke uitwerking)

- **Type:** beslissing + feature (audio/projectregel)
- **Verbetergebied:** 6 (Audio)
- **Status (historisch, ACHTERHAALD):** dit ticket is inmiddels naar de
  backlog verplaatst — zie de kop hierboven. Onderstaande statusregel
  beschrijft de stand op het moment van het regelbesluit.
